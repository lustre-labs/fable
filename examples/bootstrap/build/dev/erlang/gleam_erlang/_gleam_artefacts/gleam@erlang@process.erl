...
-opaque subject(FIO) :: {subject, pid_(), gleam@erlang:reference_()} |
    {gleam_phantom, FIO}.

-type do_not_leak() :: any().

-type selector(FIP) :: any() | {gleam_phantom, FIP}.
...
-type call_error(FIQ) :: {callee_down, gleam@dynamic:dynamic_()} |
    call_timeout |
    {gleam_phantom, FIQ}.
...
-spec 'receive'(subject(FJB), integer()) -> {ok, FJB} | {error, nil}.
...
-spec receive_forever(subject(FJF)) -> FJF.
...
-spec select(selector(FJJ), integer()) -> {ok, FJJ} | {error, nil}.
select(From, Within) ->
...
-spec select_forever(selector(FJN)) -> FJN.
select_forever(From) ->
...
-spec map_selector(selector(FJP), fun((FJP) -> FJR)) -> selector(FJR).
map_selector(A, B) ->
...
-spec merge_selector(selector(FJT), selector(FJT)) -> selector(FJT).
merge_selector(A, B) ->
...
-spec selecting_trapped_exits(selector(FJX), fun((exit_message()) -> FJX)) -> selector(FJX).
...
-spec selecting(selector(FKA), subject(FKC), fun((FKC) -> FKA)) -> selector(FKA).
...
-spec selecting_record2(
    selector(FKK),
    any(),
    fun((gleam@dynamic:dynamic_()) -> FKK)
) -> selector(FKK).
...
-spec selecting_record3(
    selector(FKO),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FKO)
) -> selector(FKO).
...
-spec selecting_record4(
    selector(FKS),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FKS)
) -> selector(FKS).
...
-spec selecting_record5(
    selector(FKW),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FKW)
) -> selector(FKW).
...
-spec selecting_record6(
    selector(FLA),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FLA)
) -> selector(FLA).
...
-spec selecting_record7(
    selector(FLE),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FLE)
) -> selector(FLE).
...
-spec selecting_record8(
    selector(FLI),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FLI)
) -> selector(FLI).
...
-spec selecting_anything(selector(FLM), fun((gleam@dynamic:dynamic_()) -> FLM)) -> selector(FLM).
...
-spec deselecting(selector(FKF), subject(any())) -> selector(FKF).
...
-spec deselecting_process_down(selector(FMB), process_monitor()) -> selector(FMB).
...
-spec try_call(subject(FME), fun((subject(FMG)) -> FME), integer()) -> {ok, FMG} |
    {error, call_error(FMG)}.
...
-spec call(subject(FML), fun((subject(FMN)) -> FML), integer()) -> FMN.
...
-spec try_call_forever(subject(FMT), fun((subject(FMV)) -> FMT)) -> {ok, FMV} |
...
-spec call_forever(subject(FMP), fun((subject(FMR)) -> FMP)) -> FMR.
...
-spec send_after(subject(FNC), integer(), FNC) -> timer().
