...
-spec set_header(request(GLB), binary(), binary()) -> request(GLB).
...
-spec prepend_header(request(GLE), binary(), binary()) -> request(GLE).
...
-spec set_body(request(any()), GLJ) -> request(GLJ).
set_body(Req, Body) ->
...
-spec map(request(GLL), fun((GLL) -> GLN)) -> request(GLN).
...
-spec set_query(request(GLX), list({binary(), binary()})) -> request(GLX).
set_query(Req, Query) ->
    Pair = fun(T) ->
...
-spec set_method(request(GMB), gleam@http:method()) -> request(GMB).
...
-spec set_scheme(request(GMI), gleam@http:scheme()) -> request(GMI).
...
-spec set_host(request(GML), binary()) -> request(GML).
set_host(Req, Host) ->
    _record = Req,
    {request,
        erlang:element(2, _record),
...
-spec set_port(request(GMO), integer()) -> request(GMO).
set_port(Req, Port) ->
    _record = Req,
    {request,
        erlang:element(2, _record),
...
-spec set_path(request(GMR), binary()) -> request(GMR).
set_path(Req, Path) ->
    _record = Req,
    {request,
        erlang:element(2, _record),
...
-spec set_cookie(request(GMU), binary(), binary()) -> request(GMU).
...
-spec remove_cookie(request(GMZ), binary()) -> request(GMZ).
