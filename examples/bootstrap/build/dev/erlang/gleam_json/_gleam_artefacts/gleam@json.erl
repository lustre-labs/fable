...
-spec parse_bits(bitstring(), gleam@dynamic@decode:decoder(EHR)) -> {ok, EHR} |
    {error, decode_error()}.
...
-spec do_parse(binary(), gleam@dynamic@decode:decoder(EHL)) -> {ok, EHL} |
    {error, decode_error()}.
...
-spec parse(binary(), gleam@dynamic@decode:decoder(EHH)) -> {ok, EHH} |
    {error, decode_error()}.
parse(Json, Decoder) ->
    do_parse(Json, Decoder).
...
-spec nullable(gleam@option:option(EHX), fun((EHX) -> json())) -> json().
...
-spec array(list(EIB), fun((EIB) -> json())) -> json().
