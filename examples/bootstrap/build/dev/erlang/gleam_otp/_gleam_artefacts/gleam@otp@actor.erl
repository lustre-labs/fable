...
-type message(HPL) :: {message, HPL} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-type next(HPM, HPN) :: {continue,
        HPN,
        gleam@option:option(gleam@erlang@process:selector(HPM))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type init_result(HPO, HPP) :: {ready, HPO, gleam@erlang@process:selector(HPP)} |
    {failed, binary()}.

-type self(HPQ, HPR) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        HPQ,
        gleam@erlang@process:subject(HPR),
        gleam@erlang@process:selector(message(HPR)),
        gleam@otp@system:debug_state(),
        fun((HPR, HPQ) -> next(HPR, HPQ))}.

-type spec(HPS, HPT) :: {spec,
        fun(() -> init_result(HPS, HPT)),
        integer(),
        fun((HPT, HPS) -> next(HPT, HPS))}.

-type start_error() :: init_timeout |
    {init_failed, gleam@erlang@process:exit_reason()} |
    {init_crashed, gleam@dynamic:dynamic_()}.

-type start_init_message(HPU) :: {ack,
        {ok, gleam@erlang@process:subject(HPU)} |
            {error, gleam@erlang@process:exit_reason()}} |
    {mon, gleam@erlang@process:process_down()}.

-file("src/gleam/otp/actor.gleam", 178).
-spec continue(HQB) -> next(any(), HQB).
continue(State) ->
    {continue, State, none}.
...
-spec with_selector(next(HQF, HQG), gleam@erlang@process:selector(HQF)) -> next(HQF, HQG).
...
-spec selecting_system_messages(gleam@erlang@process:selector(message(HQR))) -> gleam@erlang@process:selector(message(HQR)).
...
-spec receive_message(self(any(), HQN)) -> message(HQN).
...
-spec init_selector(
    gleam@erlang@process:subject(HUW),
    gleam@erlang@process:selector(HUW)
) -> gleam@erlang@process:selector(message(HUW)).
...
-spec initialise_actor(
    spec(any(), HRI),
    gleam@erlang@process:subject({ok, gleam@erlang@process:subject(HRI)} |
        {error, gleam@erlang@process:exit_reason()})
...
-spec start_spec(spec(any(), HRV)) -> {ok, gleam@erlang@process:subject(HRV)} |
    {error, start_error()}.
start_spec(Spec) ->
...
-spec start(HSB, fun((HSC, HSB) -> next(HSC, HSB))) -> {ok,
        gleam@erlang@process:subject(HSC)} |
    {error, start_error()}.
start(State, Loop) ->
    start_spec(
        {spec,
...
-spec send(gleam@erlang@process:subject(HSI), HSI) -> nil.
send(Subject, Msg) ->
...
-spec call(
    gleam@erlang@process:subject(HSK),
    fun((gleam@erlang@process:subject(HSM)) -> HSK),
    integer()
) -> HSM.
