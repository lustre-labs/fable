...
-type spec(IEP, IEQ) :: {spec,
        IEP,
        integer(),
        integer(),
        fun((children(IEP)) -> children(IEQ))}.

-opaque children(IER) :: {ready, starter(IER)} | {failed, child_start_error()}.

-opaque child_spec(IES, IET, IEU) :: {child_spec,
        fun((IET) -> {ok, gleam@erlang@process:subject(IES)} |
            {error, gleam@otp@actor:start_error()}),
        fun((IET, gleam@erlang@process:subject(IES)) -> IEU)}.
...
-spec start_child(child_spec(any(), IFB, IFC), IFB) -> {ok, child(IFC)} |
    {error, child_start_error()}.
...
-spec perform_instruction_for_child(
    IFP,
    instruction(),
    child_spec(any(), IFP, IFR),
    child(IFR)
) -> {ok, {child(IFR), instruction()}} | {error, child_start_error()}.
...
-spec start_and_add_child(starter(IGI), child_spec(any(), IGI, IGL)) -> children(IGL).
...
-spec add(children(IGQ), child_spec(any(), IGQ, IGT)) -> children(IGT).
...
-spec supervisor(
    fun((IGY) -> {ok, gleam@erlang@process:subject(IGZ)} |
        {error, gleam@otp@actor:start_error()})
) -> child_spec(IGZ, IGY, IGY).
supervisor(Start) ->
...
-spec worker(
    fun((IHG) -> {ok, gleam@erlang@process:subject(IHH)} |
        {error, gleam@otp@actor:start_error()})
) -> child_spec(IHH, IHG, IHG).
worker(Start) ->
    {child_spec, Start, fun(Argument, _) -> Argument end}.
...
-spec returning(
    child_spec(IHO, IHP, any()),
    fun((IHP, gleam@erlang@process:subject(IHO)) -> IHV)
) -> child_spec(IHO, IHP, IHV).
returning(Child, Updater) ->
    {child_spec, erlang:element(2, Child), Updater}.

-file("src/gleam/otp/supervisor.gleam", 248).
-spec init(spec(any(), IIA)) -> gleam@otp@actor:init_result(state(IIA), message()).
init(Spec) ->
...
-spec handle_exit(gleam@erlang@process:pid_(), state(IIG)) -> gleam@otp@actor:next(message(), state(IIG)).
...
-spec loop(message(), state(IIL)) -> gleam@otp@actor:next(message(), state(IIL)).
loop(Message, State) ->
    case Message of
        {exit, Exit_message} ->
