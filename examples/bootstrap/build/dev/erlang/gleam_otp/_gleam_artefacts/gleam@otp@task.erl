...
-opaque task(IQY) :: {task,
        gleam@erlang@process:pid_(),
        gleam@erlang@process:pid_(),
        gleam@erlang@process:subject(IQY)}.

-type await_error() :: timeout | {exit, gleam@dynamic:dynamic_()}.

-type message2(IQZ, IRA) :: {m2_from_subject1, IQZ} |
    {m2_from_subject2, IRA} |
    m2_timeout.

-type message3(IRB, IRC, IRD) :: {m3_from_subject1, IRB} |
    {m3_from_subject2, IRC} |
    {m3_from_subject3, IRD} |
    m3_timeout.

-type message4(IRE, IRF, IRG, IRH) :: {m4_from_subject1, IRE} |
    {m4_from_subject2, IRF} |
    {m4_from_subject3, IRG} |
    {m4_from_subject4, IRH} |
    m4_timeout.

-type message(IRI) :: {message, integer(), IRI} | message_timeout.
...
-spec try_await(task(IRN), integer()) -> {ok, IRN} | {error, await_error()}.
...
-spec await(task(IRR), integer()) -> IRR.
await(Task, Timeout) ->
...
-spec await_forever(task(IRV)) -> IRV.
await_forever(Task) ->
    assert_owner(Task),
    Selector = begin
        _pipe = gleam_erlang_ffi:new_selector(),
...
-spec try_await2_loop(
    gleam@erlang@process:selector(message2(ISF, ISG)),
    gleam@option:option({ok, ISF} | {error, await_error()}),
    gleam@option:option({ok, ISG} | {error, await_error()}),
    gleam@erlang@process:timer()
) -> {{ok, ISF} | {error, await_error()}, {ok, ISG} | {error, await_error()}}.
...
-spec try_await2(task(IRX), task(IRZ), integer()) -> {{ok, IRX} |
        {error, await_error()},
    {ok, IRZ} | {error, await_error()}}.
...
-spec try_await3_loop(
    gleam@erlang@process:selector(message3(ITG, ITH, ITI)),
    gleam@option:option({ok, ITG} | {error, await_error()}),
    gleam@option:option({ok, ITH} | {error, await_error()}),
    gleam@option:option({ok, ITI} | {error, await_error()}),
    gleam@erlang@process:timer()
) -> {{ok, ITG} | {error, await_error()},
    {ok, ITH} | {error, await_error()},
    {ok, ITI} | {error, await_error()}}.
...
-spec try_await3(task(ISU), task(ISW), task(ISY), integer()) -> {{ok, ISU} |
        {error, await_error()},
    {ok, ISW} | {error, await_error()},
    {ok, ISY} | {error, await_error()}}.
...
-spec try_await4_loop(
    gleam@erlang@process:selector(message4(IUS, IUT, IUU, IUV)),
    gleam@option:option({ok, IUS} | {error, await_error()}),
    gleam@option:option({ok, IUT} | {error, await_error()}),
    gleam@option:option({ok, IUU} | {error, await_error()}),
    gleam@option:option({ok, IUV} | {error, await_error()}),
    gleam@erlang@process:timer()
) -> {{ok, IUS} | {error, await_error()},
    {ok, IUT} | {error, await_error()},
    {ok, IUU} | {error, await_error()},
    {ok, IUV} | {error, await_error()}}.
...
-spec try_await4(task(IUC), task(IUE), task(IUG), task(IUI), integer()) -> {{ok,
            IUC} |
        {error, await_error()},
    {ok, IUE} | {error, await_error()},
    {ok, IUG} | {error, await_error()},
    {ok, IUI} | {error, await_error()}}.
...
-spec dict_to_list(gleam@dict:dict(integer(), IWL), integer(), IWL) -> list(IWL).
...
-spec try_await_all_loop(
    gleam@dict:dict(integer(), {ok, IWB} | {error, await_error()}),
    integer(),
    gleam@erlang@process:timer(),
    gleam@erlang@process:selector(message(IWB))
) -> list({ok, IWB} | {error, await_error()}).
...
-spec try_await_all(list(task(IVV)), integer()) -> list({ok, IVV} |
    {error, await_error()}).
