...
-opaque set(CZI) :: {set, gleam@dict:dict(CZI, list(nil))}.
...
-spec contains(set(CZT), CZT) -> boolean().
contains(Set, Member) ->
...
-spec delete(set(CZV), CZV) -> set(CZV).
delete(Set, Member) ->
...
-spec fold(set(DAE), DAG, fun((DAG, DAE) -> DAG)) -> DAG.
...
-spec filter(set(DAH), fun((DAH) -> boolean())) -> set(DAH).
...
-spec drop(set(DAO), list(DAO)) -> set(DAO).
drop(Set, Disallowed) ->
...
-spec take(set(DAS), list(DAS)) -> set(DAS).
take(Set, Desired) ->
    {set, gleam@dict:take(erlang:element(2, Set), Desired)}.

-file("src/gleam/set.gleam", 287).
-spec order(set(DBA), set(DBA)) -> {set(DBA), set(DBA)}.
order(First, Second) ->
...
-spec intersection(set(DBF), set(DBF)) -> set(DBF).
...
-spec difference(set(DBJ), set(DBJ)) -> set(DBJ).
...
-spec is_subset(set(DBN), set(DBN)) -> boolean().
...
-spec is_disjoint(set(DBQ), set(DBQ)) -> boolean().
...
-spec each(set(DBX), fun((DBX) -> any())) -> nil.
each(Set, Fun) ->
    fold(
        Set,
        nil,
...
-spec insert(set(CZQ), CZQ) -> set(CZQ).
insert(Set, Member) ->
...
-spec from_list(list(DAB)) -> set(DAB).
from_list(Members) ->
...
-spec map(set(DAK), fun((DAK) -> DAM)) -> set(DAM).
map(Set, Fun) ->
...
-spec union(set(DAW), set(DAW)) -> set(DAW).
union(First, Second) ->
    {Larger, Smaller} = order(First, Second),
...
-spec symmetric_difference(set(DBT), set(DBT)) -> set(DBT).
