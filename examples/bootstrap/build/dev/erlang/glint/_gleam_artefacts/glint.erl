...
-opaque glint(LKD) :: {glint, config(), command_node(LKD)}.

-type args_count() :: {eq_args, integer()} | {min_args, integer()}.

-opaque command(LKE) :: {command,
        fun((named_args(), list(binary()), flags()) -> LKE),
        flags(),
        binary(),
        gleam@option:option(args_count()),
        list(binary())}.

-type internal_command(LKF) :: {internal_command,
        fun((named_args(), list(binary()), flags()) -> LKF),
        flags(),
        gleam@option:option(args_count()),
        list(binary())}.
...
-type command_node(LKG) :: {command_node,
        gleam@option:option(internal_command(LKG)),
        gleam@dict:dict(binary(), command_node(LKG)),
        flags(),
        binary()}.

-type out(LKH) :: {out, LKH} | {help, binary()}.
...
-opaque flag(LKI) :: {flag,
        binary(),
        binary(),
        fun((binary()) -> {ok, LKI} | {error, snag:snag()}),
        fun((flag_internals(LKI)) -> value()),
        fun((flags(), binary()) -> {ok, LKI} | {error, snag:snag()}),
        gleam@option:option(LKI)}.

-type flag_internals(LKJ) :: {flag_internals,
        gleam@option:option(LKJ),
        fun((binary()) -> {ok, LKJ} | {error, snag:snag()})}.
...
-spec pretty_help(glint(LKQ), pretty_help()) -> glint(LKQ).
...
-spec with_name(glint(LKT), binary()) -> glint(LKT).
...
-spec without_exit(glint(LKW)) -> glint(LKW).
without_exit(Glint) ->
    _record = Glint,
    {glint,
        begin
...
-spec as_module(glint(LKZ)) -> glint(LKZ).
as_module(Glint) ->
    _record = Glint,
    {glint,
        begin
...
-spec with_indent_width(glint(LLC), integer()) -> glint(LLC).
...
-spec with_max_output_width(glint(LLF), integer()) -> glint(LLF).
...
-spec with_min_first_column_width(glint(LLI), integer()) -> glint(LLI).
...
-spec with_column_gap(glint(LLL), integer()) -> glint(LLL).
...
-spec global_help(glint(LLU), binary()) -> glint(LLU).
...
-spec map_command(command(LMJ), fun((LMJ) -> LML)) -> command(LML).
...
-spec command_help(binary(), fun(() -> command(LMN))) -> command(LMN).
command_help(Desc, F) ->
    _record = F(),
    {command,
...
-spec unnamed_args(args_count(), fun(() -> command(LMQ))) -> command(LMQ).
...
-spec named_arg(
    binary(),
    fun((fun((named_args()) -> binary())) -> command(LMT))
) -> command(LMT).
named_arg(Name, F) ->
    Cmd = begin
        F(
...
-spec new_builder(
    binary(),
    fun((flag_internals(LPH)) -> value()),
    fun((flags(), binary()) -> {ok, LPH} | {error, snag:snag()}),
    fun((binary()) -> {ok, LPH} | {error, snag:snag()})
) -> flag(LPH).
...
-spec wrap_with_constraint(
    fun((binary()) -> {ok, LPT} | {error, snag:snag()}),
    fun((LPT) -> {ok, LPT} | {error, snag:snag()})
) -> fun((binary()) -> {ok, LPT} | {error, snag:snag()}).
...
-spec flag_constraint(flag(LPP), fun((LPP) -> {ok, LPP} | {error, snag:snag()})) -> flag(LPP).
...
-spec flag_help(flag(LQI), binary()) -> flag(LQI).
...
-spec flag_default(flag(LQL), LQL) -> flag(LQL).
...
-spec flag(
    flag(LMW),
    fun((fun((flags()) -> {ok, LMW} | {error, snag:snag()})) -> command(LMZ))
) -> command(LMZ).
flag(Flag, F) ->
    Cmd = F(
        fun(_capture) ->
...
-spec fold(flags(), LQO, fun((LQO, binary(), flag_entry()) -> LQO)) -> LQO.
fold(Flags, Acc, F) ->
...
-spec command(fun((named_args(), list(binary()), flags()) -> LMG)) -> command(LMG).
command(Runner) ->
...
-spec get_value(
    flags(),
    binary(),
    fun((flag_entry()) -> {ok, LRB} | {error, snag:snag()})
) -> {ok, LRB} | {error, snag:snag()}.
...
-spec get_flag(flags(), flag(LRE)) -> {ok, LRE} | {error, snag:snag()}.
get_flag(Flags, Flag) ->
...
-spec do_update_at(
    command_node(LRX),
    list(binary()),
    fun((command_node(LRX)) -> command_node(LRX))
) -> command_node(LRX).
...
-spec update_at(
    glint(LRR),
    list(binary()),
    fun((command_node(LRR)) -> command_node(LRR))
) -> glint(LRR).
...
-spec path_help(glint(LLQ), list(binary()), binary()) -> glint(LLQ).
...
-spec add(glint(LLX), list(binary()), command(LLX)) -> glint(LLX).
...
-spec group_flag(glint(LNC), list(binary()), flag(any())) -> glint(LNC).
...
?DOC(false).
-spec execute(glint(LNI), list(binary())) -> {ok, out(LNI)} | {error, binary()}.
execute(Glint, Args) ->
    Help_flag = <<"--"/utf8,
        (erlang:element(
            2,
...
-spec run_and_handle(glint(LOH), list(binary()), fun((LOH) -> any())) -> nil.
