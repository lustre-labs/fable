...
-spec one_of(list(LDT)) -> fun((LDT) -> {ok, LDT} | {error, snag:snag()}).
one_of(Allowed) ->
...
-spec none_of(list(LDW)) -> fun((LDW) -> {ok, LDW} | {error, snag:snag()}).
none_of(Disallowed) ->
...
-spec each(fun((LDZ) -> {ok, LDZ} | {error, snag:snag()})) -> fun((list(LDZ)) -> {ok,
        list(LDZ)} |
    {error, snag:snag()}).
each(Constraint) ->
