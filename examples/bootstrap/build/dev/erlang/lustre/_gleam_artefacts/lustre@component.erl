...
-opaque config(UNI) :: {config,
        boolean(),
        boolean(),
        gleam@dict:dict(binary(), fun((binary()) -> {ok, UNI} | {error, nil})),
        gleam@dict:dict(binary(), gleam@dynamic@decode:decoder(UNI)),
        boolean(),
        gleam@option:option(fun((binary()) -> UNI)),
        gleam@option:option(UNI),
        gleam@option:option(fun((binary()) -> UNI))}.

-opaque option(UNJ) :: {option, fun((config(UNJ)) -> config(UNJ))}.

-file("src/lustre/component.gleam", 131).
?DOC(false).
-spec new(list(option(UNK))) -> config(UNK).
new(Options) ->
    Init = {config,
        false,
        true,
...
-spec on_attribute_change(binary(), fun((binary()) -> {ok, UNO} | {error, nil})) -> option(UNO).
...
-spec on_property_change(binary(), gleam@dynamic@decode:decoder(UNS)) -> option(UNS).
...
-spec on_form_autofill(fun((binary()) -> UNX)) -> option(UNX).
...
-spec on_form_reset(UNZ) -> option(UNZ).
on_form_reset(Msg) ->
    {option, fun(Config) -> _record = Config,
            {config,
                erlang:element(2, _record),
...
-spec on_form_restore(fun((binary()) -> UOB)) -> option(UOB).
...
?DOC(false).
-spec to_server_component_config(config(UOH)) -> lustre@runtime@server@runtime:config(UOH).
...
-spec default_slot(
    list(lustre@vdom@vattr:attribute(UOK)),
    list(lustre@vdom@vnode:element(UOK))
) -> lustre@vdom@vnode:element(UOK).
...
-spec named_slot(
    binary(),
    list(lustre@vdom@vattr:attribute(UOQ)),
    list(lustre@vdom@vnode:element(UOQ))
) -> lustre@vdom@vnode:element(UOQ).
