...
-spec extract_keyed_children(list({binary(), lustre@vdom@vnode:element(WAH)})) -> {lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(WAH)),
    list(lustre@vdom@vnode:element(WAH)),
    integer()}.
...
-spec element(
    binary(),
    list(lustre@vdom@vattr:attribute(VYN)),
    list({binary(), lustre@vdom@vnode:element(VYN)})
) -> lustre@vdom@vnode:element(VYN).
...
-spec namespaced(
    binary(),
    binary(),
    list(lustre@vdom@vattr:attribute(VYT)),
    list({binary(), lustre@vdom@vnode:element(VYT)})
) -> lustre@vdom@vnode:element(VYT).
...
-spec fragment(list({binary(), lustre@vdom@vnode:element(VYZ)})) -> lustre@vdom@vnode:element(VYZ).
fragment(Children) ->
...
-spec ul(
    list(lustre@vdom@vattr:attribute(VZD)),
    list({binary(), lustre@vdom@vnode:element(VZD)})
) -> lustre@vdom@vnode:element(VZD).
ul(Attributes, Children) ->
    element(<<"ul"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 155).
-spec ol(
    list(lustre@vdom@vattr:attribute(VZJ)),
    list({binary(), lustre@vdom@vnode:element(VZJ)})
) -> lustre@vdom@vnode:element(VZJ).
ol(Attributes, Children) ->
    element(<<"ol"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 162).
-spec 'div'(
    list(lustre@vdom@vattr:attribute(VZP)),
    list({binary(), lustre@vdom@vnode:element(VZP)})
) -> lustre@vdom@vnode:element(VZP).
'div'(Attributes, Children) ->
    element(<<"div"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 169).
-spec tbody(
    list(lustre@vdom@vattr:attribute(VZV)),
    list({binary(), lustre@vdom@vnode:element(VZV)})
) -> lustre@vdom@vnode:element(VZV).
tbody(Attributes, Children) ->
    element(<<"tbody"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 176).
-spec dl(
    list(lustre@vdom@vattr:attribute(WAB)),
    list({binary(), lustre@vdom@vnode:element(WAB)})
) -> lustre@vdom@vnode:element(WAB).
