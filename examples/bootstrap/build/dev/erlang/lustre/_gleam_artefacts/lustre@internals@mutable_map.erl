...
-type mutable_map(QKF, QKG) :: any() | {gleam_phantom, QKF, QKG}.
...
?DOC(false).
-spec get(mutable_map(QKL, QKM), QKL) -> {ok, QKM} | {error, nil}.
get(Map, Key) ->
    gleam@dict:get(Map, Key).

-file("src/lustre/internals/mutable_map.gleam", 33).
?DOC(false).
-spec insert(mutable_map(QKR, QKS), QKR, QKS) -> mutable_map(QKR, QKS).
insert(Map, Key, Value) ->
    gleam@dict:insert(Map, Key, Value).

-file("src/lustre/internals/mutable_map.gleam", 43).
?DOC(false).
-spec delete(mutable_map(QKX, QKY), QKX) -> mutable_map(QKX, QKY).
delete(Map, Key) ->
