...
-opaque events(QVO) :: {events,
        lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(QVO)),
        list(binary()),
        list(binary())}.
...
?DOC(false).
-spec tick(events(QVU)) -> events(QVU).
tick(Events) ->
...
-spec do_remove_event(
    lustre@internals@mutable_map:mutable_map(binary(), QWM),
...
?DOC(false).
-spec remove_event(events(QWJ), lustre@vdom@path:path(), binary()) -> events(QWJ).
...
-spec remove_attributes(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(QYM)),
    lustre@vdom@path:path(),
    list(lustre@vdom@vattr:attribute(QYM))
) -> lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(QYM)).
...
?DOC(false).
-spec handle(events(QZE), binary(), binary(), gleam@dynamic:dynamic_()) -> {events(QZE),
    {ok, QZE} | {error, list(gleam@dynamic@decode:decode_error())}}.
...
?DOC(false).
-spec do_add_event(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(QWB)),
    fun((gleam@dynamic:dynamic_()) -> gleam@dynamic:dynamic_()),
    lustre@vdom@path:path(),
    binary(),
    gleam@dynamic@decode:decoder(QWB)
) -> lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(QWB)).
...
-spec add_event(
    ...
    lustre@vdom@path:path(),
    binary(),
    gleam@dynamic@decode:decoder(QVX)
) -> events(QVX).
...
?DOC(false).
-spec add_attributes(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(QXD)),
    fun((gleam@dynamic:dynamic_()) -> gleam@dynamic:dynamic_()),
    lustre@vdom@path:path(),
    list(lustre@vdom@vattr:attribute(QXD))
) -> lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(QXD)).
...
?DOC(false).
-spec is_reference_equal(QZP, QZP) -> boolean().
...
-spec do_remove_children(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(QYV)),
    lustre@vdom@path:path(),
    integer(),
    list(lustre@vdom@vnode:element(QYV))
) -> lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(QYV)).
...
-spec do_remove_child(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(QYE)),
    lustre@vdom@path:path(),
    integer(),
    lustre@vdom@vnode:element(QYE)
) -> lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(QYE)).
...
-spec do_add_children(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(QXR)),
    fun((gleam@dynamic:dynamic_()) -> gleam@dynamic:dynamic_()),
    lustre@vdom@path:path(),
    integer(),
    list(lustre@vdom@vnode:element(QXR))
) -> lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(QXR)).
...
?DOC(false).
-spec do_add_child(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(QWV)),
    fun((gleam@dynamic:dynamic_()) -> gleam@dynamic:dynamic_()),
    lustre@vdom@path:path(),
    integer(),
...
?DOC(false).
-spec from_node(lustre@vdom@vnode:element(QVR)) -> events(QVR).
from_node(Root) ->
...
-spec add_children(
    ...
    lustre@vdom@path:path(),
    integer(),
    list(lustre@vdom@vnode:element(QXM))
) -> events(QXM).
