...
-opaque cli(AFVQ) :: {cli,
        fun((env()) -> {env(),
            {ok, AFVQ} | {error, lustre_dev_tools@error:error()}})}.
...
?DOC(false).
-spec run(cli(AFVR), glint:flags()) -> {ok, AFVR} |
...
?DOC(false).
-spec from_result({ok, AFVZ} | {error, lustre_dev_tools@error:error()}) -> cli(AFVZ).
from_result(Result) ->
    {cli, fun(Env) -> {Env, Result} end}.

-file("src/lustre_dev_tools/cli.gleam", 88).
?DOC(false).
-spec do(cli(AFWD), fun((AFWD) -> cli(AFWF))) -> cli(AFWF).
do(Step, Next) ->
    {cli,
        fun(Env) ->
            {Env@1, Result} = (erlang:element(2, Step))(Env),
...
?DOC(false).
-spec in(fun(() -> AFWI)) -> cli(AFWI).
in(Value) ->
    {cli, fun(Env) -> {Env, {ok, Value()}} end}.

-file("src/lustre_dev_tools/cli.gleam", 111).
?DOC(false).
-spec map(cli(AFWK), fun((AFWK) -> AFWM)) -> cli(AFWM).
map(Step, Next) ->
    {cli,
        fun(Env) ->
            {Env@1, Result} = (erlang:element(2, Step))(Env),
...
?DOC(false).
-spec 'try'(
    {ok, AFWO} | {error, lustre_dev_tools@error:error()},
    fun((AFWO) -> cli(AFWR))
) -> cli(AFWR).
'try'(Result, Next) ->
...
?DOC(false).
-spec log(binary(), fun(() -> cli(AFWU))) -> cli(AFWU).
log(Message, Next) ->
    {cli,
        fun(Env) ->
...
?DOC(false).
-spec success(binary(), fun(() -> cli(AFWX))) -> cli(AFWX).
...
?DOC(false).
-spec notify(binary(), fun(() -> cli(AFXA))) -> cli(AFXA).
notify(Message, Next) ->
    {cli,
        fun(Env) ->
            Env@1 = begin
...
?DOC(false).
-spec template(binary(), fun((binary()) -> cli(AFXF))) -> cli(AFXF).
template(Name, Next) ->
    {cli,
        fun(Env) ->
...
-spec get_config_value(
    ...
    AFXL,
    list(binary()),
    fun((gleam@dict:dict(binary(), tom:toml()), list(binary())) -> {ok, AFXL} |
        {error, any()}),
    fun((glint:flags()) -> {ok, AFXL} | {error, any()})
) -> cli(AFXL).
