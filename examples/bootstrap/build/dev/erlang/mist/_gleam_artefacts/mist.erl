...
-opaque builder(AAPP, AAPQ) :: {builder,
        integer(),
        fun((gleam@http@request:request(AAPP)) -> gleam@http@response:response(AAPQ)),
...
-type websocket_message(AAPR) :: {text, binary()} |
    {binary, bitstring()} |
    closed |
    shutdown |
    {custom, AAPR}.
...
-spec new(
    fun((gleam@http@request:request(AAQO)) -> gleam@http@response:response(AAQQ))
) -> builder(AAQO, AAQQ).
new(Handler) ->
    {builder,
        4000,
...
-spec port(builder(AAQU, AAQV), integer()) -> builder(AAQU, AAQV).
port(Builder, Port) ->
    _record = Builder,
    {builder,
        Port,
...
-spec read_request_body(
    builder(bitstring(), AARA),
    integer(),
    gleam@http@response:response(AARA)
) -> builder(mist@internal@http:connection(), AARA).
...
-spec bind(builder(AARM, AARN), binary()) -> builder(AARM, AARN).
...
-spec with_ipv6(builder(AARS, AART)) -> builder(AARS, AART).
with_ipv6(Builder) ->
    _record = Builder,
    {builder,
        erlang:element(2, _record),
...
-spec internal_to_public_ws_message(
    mist@internal@websocket:handler_message(AAST)
) -> {ok, websocket_message(AAST)} | {error, nil}.
...
-spec websocket(
    gleam@http@request:request(mist@internal@http:connection()),
    fun((AASZ, mist@internal@websocket:websocket_connection(), websocket_message(AATA)) -> gleam@otp@actor:next(AATA, AASZ)),
    fun((mist@internal@websocket:websocket_connection()) -> {AASZ,
        gleam@option:option(gleam@erlang@process:selector(AATA))}),
    fun((AASZ) -> nil)
...
-spec server_sent_events(
    ...
    fun(() -> gleam@otp@actor:init_result(AATO, AATP)),
    fun((AATP, sse_connection(), AATO) -> gleam@otp@actor:next(AATP, AATO))
) -> gleam@http@response:response(response_data()).
