...
-spec init(fun((gleam@uri:uri()) -> AHYO)) -> lustre@effect:effect(AHYO).
init(Handler) ->
    lustre@effect:from(
        fun(Dispatch) ->
            gleam@bool:guard(
...
-spec advanced(options(), fun((gleam@uri:uri()) -> AHYQ)) -> lustre@effect:effect(AHYQ).
...
-spec simulate(
    lustre@dev@simulate:simulation(AHZG, AHZH),
    lustre@dev@query:'query'(),
    binary(),
    fun((gleam@uri:uri()) -> AHZH)
) -> lustre@dev@simulate:simulation(AHZG, AHZH).
