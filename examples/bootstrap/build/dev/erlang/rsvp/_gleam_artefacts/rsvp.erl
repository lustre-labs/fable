...
-opaque handler(AIFA) :: {handler,
        fun(({ok, gleam@http@response:response(binary())} | {error, error()}) -> AIFA)}.
...
-spec expect_ok_response(
    fun(({ok, gleam@http@response:response(binary())} | {error, error()}) -> AIFY)
) -> handler(AIFY).
...
-spec expect_json_response(
    fun(({ok, gleam@http@response:response(binary())} | {error, error()}) -> AIFK)
) -> handler(AIFK).
...
-spec expect_text_response(
    fun(({ok, gleam@http@response:response(binary())} | {error, error()}) -> AIFT)
) -> handler(AIFT).
...
-spec expect_text(fun(({ok, binary()} | {error, error()}) -> AIFO)) -> handler(AIFO).
expect_text(Handler) ->
...
-spec expect_any_response(
    fun(({ok, gleam@http@response:response(binary())} | {error, error()}) -> AIGD)
) -> handler(AIGD).
expect_any_response(Handler) ->
    {handler, Handler}.

-file("src/rsvp.gleam", 300).
-spec do_send(gleam@http@request:request(binary()), handler(AIGQ)) -> lustre@effect:effect(AIGQ).
...
-spec send(gleam@http@request:request(binary()), handler(AIGM)) -> lustre@effect:effect(AIGM).
...
-spec simulate(
    lustre@dev@simulate:simulation(AIGT, AIGU),
...
-spec reject(error(), handler(AIHB)) -> lustre@effect:effect(AIHB).
reject(Err, Handler) ->
...
-spec decode_json_body(
    gleam@http@response:response(binary()),
    gleam@dynamic@decode:decoder(AIHF)
) -> {ok, AIHF} | {error, error()}.
...
-spec expect_json(
    gleam@dynamic@decode:decoder(AIFB),
    fun(({ok, AIFB} | {error, error()}) -> AIFF)
) -> handler(AIFF).
...
-spec get(binary(), handler(AIGF)) -> lustre@effect:effect(AIGF).
get(Url, Handler) ->
    case to_uri(Url) of
        {ok, Uri} ->
...
-spec post(binary(), gleam@json:json(), handler(AIGI)) -> lustre@effect:effect(AIGI).
