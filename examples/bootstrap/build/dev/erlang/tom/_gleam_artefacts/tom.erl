...
-spec push_key({ok, ABTK} | {error, get_error()}, binary()) -> {ok, ABTK} |
    {error, get_error()}.
push_key(Result, Key) ->
    case Result of
        {ok, T} ->
...
-spec expect_end_of_line(
    list(binary()),
    fun((list(binary())) -> {ok, {ABVP, list(binary())}} |
        {error, parse_error()})
) -> {ok, {ABVP, list(binary())}} | {error, parse_error()}.
...
-spec do(
    {ok, {ABWA, list(binary())}} | {error, parse_error()},
    fun((ABWA, list(binary())) -> {ok, ABWD} | {error, parse_error()})
) -> {ok, ABWD} | {error, parse_error()}.
do(Result, Next) ->
    case Result of
        {ok, {A, Input}} ->
            Next(A, Input);
...
-spec expect(
    list(binary()),
    binary(),
    fun((list(binary())) -> {ok, {ABWI, list(binary())}} |
        {error, parse_error()})
) -> {ok, {ABWI, list(binary())}} | {error, parse_error()}.
