...
-spec method_override(gleam@http@request:request(AEPV)) -> gleam@http@request:request(AEPV).
...
-spec fn_with_bad_request_error(fun((AEQW) -> {ok, AEQX} | {error, any()})) -> fun((AEQW) -> {ok,
        AEQX} |
    {error, gleam@http@response:response(body())}).
...
-spec multipart_body(
    ...
    integer(),
    integer(),
    fun((AEQQ, bitstring()) -> {ok, AEQQ} |
        {error, gleam@http@response:response(body())}),
    AEQQ
) -> {ok, {gleam@option:option(buffered_reader()), integer(), AEQQ}} |
    {error, gleam@http@response:response(body())}.
...
-spec sort_keys(list({binary(), AERO})) -> list({binary(), AERO}).
sort_keys(Pairs) ->
    gleam@list:sort(
        Pairs,
        fun(A, B) ->
...
-spec or_400(
    {ok, AERR} | {error, any()},
    fun((AERR) -> gleam@http@response:response(body()))
) -> gleam@http@response:response(body()).
or_400(Result, Next) ->
    case Result of
        {ok, Value} ->
...
-spec or_500({ok, AEQI} | {error, any()}) -> {ok, AEQI} |
    {error, gleam@http@response:response(body())}.
or_500(Result) ->
    case Result of
        {ok, Value} ->
