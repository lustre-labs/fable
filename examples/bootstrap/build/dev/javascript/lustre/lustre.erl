...
-opaque app(SYN, SYO, SYP) :: {app,
        fun((SYN) -> {SYO, lustre@effect:effect(SYP)}),
        fun((SYO, SYP) -> {SYO, lustre@effect:effect(SYP)}),
        fun((SYO) -> lustre@vdom@vnode:element(SYP)),
        lustre@component:config(SYP)}.
...
-type runtime(SYQ) :: any() | {gleam_phantom, SYQ}.
...
-spec component(
    fun((SZP) -> {SZQ, lustre@effect:effect(SZR)}),
    fun((SZQ, SZR) -> {SZQ, lustre@effect:effect(SZR)}),
    fun((SZQ) -> lustre@vdom@vnode:element(SZR)),
    list(lustre@component:option(SZR))
) -> app(SZP, SZQ, SZR).
...
-spec application(
    fun((SZG) -> {SZH, lustre@effect:effect(SZI)}),
    fun((SZH, SZI) -> {SZH, lustre@effect:effect(SZI)}),
    fun((SZH) -> lustre@vdom@vnode:element(SZI))
) -> app(SZG, SZH, SZI).
...
-spec element(lustre@vdom@vnode:element(SYT)) -> app(any(), nil, SYT).
element(View) ->
    application(
...
-spec simple(
    fun((SYZ) -> SZA),
    fun((SZA, SZB) -> SZA),
    fun((SZA) -> lustre@vdom@vnode:element(SZB))
) -> app(SYZ, SZA, SZB).
...
-spec do_start(app(TAJ, any(), TAL), binary(), TAJ) -> {ok, runtime(TAL)} |
    {error, error()}.
do_start(_, _, _) ->
    {error, not_a_browser}.
...
-spec send(runtime(TBI), lustre@runtime@server@runtime:message(TBI)) -> nil.
...
-spec dispatch(TBL) -> lustre@runtime@server@runtime:message(TBL).
dispatch(Msg) ->
...
-spec start(app(TAA, any(), TAC), binary(), TAA) -> {ok, runtime(TAC)} |
    {error, error()}.
...
-spec start_server_component(app(TAS, any(), TAU), TAS) -> {ok, runtime(TAU)} |
    {error, error()}.
