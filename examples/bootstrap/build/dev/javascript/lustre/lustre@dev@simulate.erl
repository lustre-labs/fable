...
-opaque app(TTG, TTH, TTI) :: {app,
        fun((TTG) -> {TTH, lustre@effect:effect(TTI)}),
        fun((TTH, TTI) -> {TTH, lustre@effect:effect(TTI)}),
        fun((TTH) -> lustre@vdom@vnode:element(TTI))}.

-opaque simulation(TTJ, TTK) :: {simulation,
        fun((TTJ, TTK) -> {TTJ, lustre@effect:effect(TTK)}),
        fun((TTJ) -> lustre@vdom@vnode:element(TTK)),
        list(event(TTK)),
        TTJ,
        lustre@vdom@vnode:element(TTK)}.

-type event(TTL) :: {dispatch, TTL} |
...
-spec simple(
    fun((TTM) -> TTN),
    fun((TTN, TTO) -> TTN),
    fun((TTN) -> lustre@vdom@vnode:element(TTO))
) -> app(TTM, TTN, TTO).
...
-spec application(
    fun((TTT) -> {TTU, lustre@effect:effect(TTV)}),
    fun((TTU, TTV) -> {TTU, lustre@effect:effect(TTV)}),
    fun((TTU) -> lustre@vdom@vnode:element(TTV))
) -> app(TTT, TTU, TTV).
...
-spec start(app(TUC, TUD, TUE), TUC) -> simulation(TUD, TUE).
start(App, Args) ->
...
-spec message(simulation(TUK, TUL), TUL) -> simulation(TUK, TUL).
...
-spec problem(simulation(TVQ, TVR), binary(), binary()) -> simulation(TVQ, TVR).
...
-spec model(simulation(TVW, any())) -> TVW.
model(Simulation) ->
    erlang:element(5, Simulation).
...
-spec view(simulation(any(), TWB)) -> lustre@vdom@vnode:element(TWB).
view(Simulation) ->
...
-spec history(simulation(any(), TWG)) -> list(event(TWG)).
history(Simulation) ->
...
-spec click(simulation(TUX, TUY), lustre@dev@query:'query'()) -> simulation(TUX, TUY).
...
-spec input(simulation(TVD, TVE), lustre@dev@query:'query'(), binary()) -> simulation(TVD, TVE).
