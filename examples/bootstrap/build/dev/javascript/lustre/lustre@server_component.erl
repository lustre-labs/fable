...
-spec element(
    list(lustre@vdom@vattr:attribute(VCS)),
    list(lustre@vdom@vnode:element(VCS))
) -> lustre@vdom@vnode:element(VCS).
...
-spec include(lustre@vdom@vattr:attribute(VDE), list(binary())) -> lustre@vdom@vattr:attribute(VDE).
...
-spec subject(lustre:runtime(VDI)) -> gleam@erlang@process:subject(lustre@runtime@server@runtime:message(VDI)).
subject(Runtime) ->
...
-spec register_subject(
    gleam@erlang@process:subject(lustre@runtime@transport:client_message(VDQ))
) -> lustre@runtime@server@runtime:message(VDQ).
...
-spec deregister_subject(
    gleam@erlang@process:subject(lustre@runtime@transport:client_message(VDU))
) -> lustre@runtime@server@runtime:message(VDU).
...
-spec register_callback(
    fun((lustre@runtime@transport:client_message(VDY)) -> nil)
) -> lustre@runtime@server@runtime:message(VDY).
...
-spec deregister_callback(
    fun((lustre@runtime@transport:client_message(VEB)) -> nil)
) -> lustre@runtime@server@runtime:message(VEB).
...
-spec select(
    fun((fun((VEG) -> nil), gleam@erlang@process:subject(any())) -> gleam@erlang@process:selector(VEG))
) -> lustre@effect:effect(VEG).
select(Sel) ->
