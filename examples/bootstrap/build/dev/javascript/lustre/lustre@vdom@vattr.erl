...
-type attribute(NJW) :: {attribute, integer(), binary(), binary()} |
    {property, integer(), binary(), gleam@json:json()} |
    {event,
        integer(),
        binary(),
        gleam@dynamic@decode:decoder(NJW),
        list(binary()),
        boolean(),
        boolean(),
        boolean(),
...
?DOC(false).
-spec merge(list(attribute(NKK)), list(attribute(NKK))) -> list(attribute(NKK)).
...
?DOC(false).
-spec compare(attribute(NKR), attribute(NKR)) -> gleam@order:order().
compare(A, B) ->
    gleam@string:compare(erlang:element(3, A), erlang:element(3, B)).

-file("src/lustre/vdom/vattr.gleam", 72).
?DOC(false).
-spec prepare(list(attribute(NKF))) -> list(attribute(NKF)).
prepare(Attributes) ->
    case Attributes of
        [] ->
            Attributes;
...
-spec event(
    binary(),
    gleam@dynamic@decode:decoder(NKB),
    list(binary()),
    boolean(),
    boolean(),
    boolean(),
