...
-type valid_message(OST) :: {socket_message, bitstring()} |
    socket_closed_message |
    {user_message, OST}.

-type websocket_message(OSU) :: {valid, valid_message(OSU)} | invalid.
...
-type handler_message(OSV) :: {internal, gramps@websocket:frame()} | {user, OSV}.

-type websocket_state(OSW) :: {websocket_state,
        bitstring(),
        OSW,
...
?DOC(false).
-spec map_user_selector(gleam@option:option(gleam@erlang@process:selector(OUE))) -> gleam@option:option(gleam@erlang@process:selector(websocket_message(OUE))).
...
-spec apply_frames(
    ...
    fun((OTU, websocket_connection(), handler_message(OTV)) -> gleam@otp@actor:next(OTV, OTU)),
    websocket_connection(),
    gleam@otp@actor:next(websocket_message(OTV), OTU),
    fun((OTU) -> nil)
) -> gleam@otp@actor:next(websocket_message(OTV), OTU).
...
-spec initialize_connection(
    fun((websocket_connection()) -> {OTF,
        gleam@option:option(gleam@erlang@process:selector(OTG))}),
    fun((OTF) -> nil),
    fun((OTF, websocket_connection(), handler_message(OTG)) -> gleam@otp@actor:next(OTG, OTF)),
    glisten@socket:socket(),
    glisten@transport:transport(),
    list(binary())
) -> {ok, gleam@erlang@process:subject(websocket_message(OTG))} | {error, nil}.
