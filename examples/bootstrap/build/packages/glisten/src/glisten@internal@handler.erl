...
-type message(HWX) :: {internal, internal_message()} | {user, HWX}.

-type loop_message(HWY) :: {packet, bitstring()} | {custom, HWY}.

-type loop_state(HWZ, HXA) :: {loop_state,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        gleam@erlang@process:subject(message(HWZ)),
        glisten@transport:transport(),
        HXA}.

-type connection(HXB) :: {connection,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(message(HXB))}.

-type handler(HXC, HXD) :: {handler,
        glisten@socket:socket(),
        fun((loop_message(HXC), HXD, connection(HXC)) -> gleam@otp@actor:next(loop_message(HXC), HXD)),
        fun((connection(HXC)) -> {HXD,
            gleam@option:option(gleam@erlang@process:selector(HXC))}),
        gleam@option:option(fun((HXD) -> nil)),
        glisten@transport:transport()}.

-file("/home/alex/gleams/glisten/src/glisten/internal/handler.gleam", 73).
-spec start(handler(HXN, any())) -> {ok,
        gleam@erlang@process:subject(message(HXN))} |
