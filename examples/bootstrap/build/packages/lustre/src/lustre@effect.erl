...
-opaque effect(OHV) :: {effect,
        list(fun((actions(OHV)) -> nil)),
        list(fun((actions(OHV)) -> nil)),
        list(fun((actions(OHV)) -> nil))}.

-type actions(OHW) :: {actions,
        fun((OHW) -> nil),
        fun((binary(), gleam@json:json()) -> nil),
        fun((gleam@erlang@process:selector(OHW)) -> nil),
        fun(() -> gleam@dynamic:dynamic_())}.

-file("src/lustre/effect.gleam", 299).
-spec do_comap_select(
    actions(OJE),
    gleam@erlang@process:selector(OJG),
    fun((OJG) -> OJE)
) -> nil.
...
-spec do_comap_actions(actions(OJA), fun((OJC) -> OJA)) -> actions(OJC).
...
-spec do_map(list(fun((actions(OIU)) -> nil)), fun((OIU) -> OIX)) -> list(fun((actions(OIX)) -> nil)).
do_map(Effects, F) ->
    gleam@list:map(
        Effects,
        fun(Effect) ->
...
-spec map(effect(OIQ), fun((OIQ) -> OIS)) -> effect(OIS).
map(Effect, F) ->
    {effect,
...
-spec perform(
    ...
    fun((OJI) -> nil),
    fun((binary(), gleam@json:json()) -> nil),
    fun((gleam@erlang@process:selector(OJI)) -> nil),
    fun(() -> gleam@dynamic:dynamic_())
) -> nil.
...
-spec from(fun((fun((OHZ) -> nil)) -> nil)) -> effect(OHZ).
from(Effect) ->
    Task = fun(Actions) ->
...
-spec before_paint(fun((fun((OIB) -> nil), gleam@dynamic:dynamic_()) -> nil)) -> effect(OIB).
before_paint(Effect) ->
    Task = fun(Actions) ->
...
-spec after_paint(fun((fun((OID) -> nil), gleam@dynamic:dynamic_()) -> nil)) -> effect(OID).
after_paint(Effect) ->
    Task = fun(Actions) ->
        Root = (erlang:element(5, Actions))(),
        Dispatch = erlang:element(2, Actions),
...
?DOC(false).
-spec select(
    fun((fun((OIH) -> nil), gleam@erlang@process:subject(any())) -> gleam@erlang@process:selector(OIH))
) -> effect(OIH).
select(Sel) ->
    Task = fun(Actions) ->
...
-spec batch(list(effect(OIM))) -> effect(OIM).
batch(Effects) ->
    gleam@list:fold(
        Effects,
        {effect, [], [], []},
