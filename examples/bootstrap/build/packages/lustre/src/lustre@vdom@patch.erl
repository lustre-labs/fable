...
-type diff(QTT) :: {diff, patch(QTT), lustre@vdom@events:events(QTT)}.

-type patch(QTU) :: {patch,
        integer(),
        integer(),
        list(change(QTU)),
        list(patch(QTU))}.

-type change(QTV) :: {replace_text, integer(), binary()} |
    {replace_inner_html, integer(), binary()} |
    {update,
        integer(),
        list(lustre@vdom@vattr:attribute(QTV)),
        list(lustre@vdom@vattr:attribute(QTV))} |
    {move, integer(), binary(), integer(), integer()} |
    {remove_key, integer(), binary(), integer()} |
    {replace, integer(), integer(), integer(), lustre@vdom@vnode:element(QTV)} |
    {insert, integer(), list(lustre@vdom@vnode:element(QTV)), integer()} |
    {remove, integer(), integer(), integer()}.

-file("src/lustre/vdom/patch.gleam", 86).
?DOC(false).
-spec new(integer(), integer(), list(change(QTW)), list(patch(QTW))) -> patch(QTW).
...
?DOC(false).
-spec add_child(patch(QVB), patch(QVB)) -> patch(QVB).
...
?DOC(false).
-spec update(
    list(lustre@vdom@vattr:attribute(QUG)),
    list(lustre@vdom@vattr:attribute(QUG))
) -> change(QUG).
...
?DOC(false).
-spec replace(integer(), integer(), lustre@vdom@vnode:element(QUQ)) -> change(QUQ).
replace(From, Count, With) ->
    {replace, 5, From, Count, With}.

-file("src/lustre/vdom/patch.gleam", 144).
?DOC(false).
-spec insert(list(lustre@vdom@vnode:element(QUT)), integer()) -> change(QUT).
